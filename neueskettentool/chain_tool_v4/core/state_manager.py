...
class StateManager:
    ...
    def save_to_scene(self) -> None:
        ...
        bpy.context.scene['chain_tool_state'] = json.dumps(state_data)
    
    def load_from_scene(self) -> None:
        """Load state from Blender scene"""
        if not bpy.context.scene or 'chain_tool_state' not in bpy.context.scene:
            return
        
        try:
            state_data = json.loads(bpy.context.scene['chain_tool_state'])
