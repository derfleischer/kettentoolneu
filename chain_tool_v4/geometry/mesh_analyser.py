...
class MeshAnalyzer:
    ...
    def find_nearest_range(self, point: Vector, max_dist: float) -> List[Tuple[Vector, Vector, int, float]]:
        """
        ...
        Returns:
            List of (location, normal, face_index, distance) tuples
        """
        if not self.bvh_tree:
            return []
        
        return self.bvh_tree.find_nearest_range(point, max_dist)
    
    def overlap_sphere(self, center: Vector, radius: float) -> List[Tuple[int, Vector]]:
        """
        Find faces overlapping with sphere
        ...
        Returns:
            List of (face_index, nearest_point) tuples
        """
        if not self.bvh_tree:
            return []
        
        # BVHTree.overlap doesn't exist, use find_nearest_range instead
        results = []
        hits = self.find_nearest_range(center, radius)
        
        for location, normal, face_idx, dist in hits:
            results.append((face_idx, location))
        
        return results
    
    def get_face_center(self, face_index: int) -> Optional[Vector]:
        """
        ...
        """
        if face_index < 0 or face_index >= len(self.mesh_data.faces):
            return None
        
        face = self.mesh_data.faces[face_index]
        if not face:
            return None
        
        # Calculate average of vertices
        center = Vector((0, 0, 0))
        for vert_idx in face:
            if vert_idx < len(self.mesh_data.vertices):
                center += self.mesh_data.vertices[vert_idx]
        
        return center / len(face) if face else None
    
    def get_face_normal(self, face_index: int) -> Optional[Vector]:
        """
        ...
        Returns:
            Normal vector or None
        """
        if face_index < 0 or face_index >= len(self.mesh_data.normals):
            return None
        
        return self.mesh_data.normals[face_index].copy()
